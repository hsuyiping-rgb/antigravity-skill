...
沒做到的項目用 ⚠️ 或 ❌ 並說明原因。若本次改過 `~/.claude/`／`C:\Users\vm\.gemini\config\skills\` 的全域設定或技能，提醒跑 chezmoi 同步（`chezmoi re-add` ＋ push）。
...
- 本 skill 在 `C:\Users\vm\.gemini\config\skills\`，搭配 chezmoi 跨電腦同步；修改後記得 `chezmoi re-add C:\Users\vm\.gemini\config\skills\shutdown/SKILL.md`
