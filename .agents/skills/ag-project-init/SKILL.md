...
> 為什麼藍圖叫 `agents.md` 而不是 `CLAUDE.md`？因為 AGENTS.md 是跨 Agent 開放標準——Antigravity、Codex、Gemini CLI、OpenCode 都讀得懂。專案層的檔案刻意用開放格式，任何 Agent 接手都能無縫工作。
...
6. **git 初始化**：
   ```bash
   git init
   git config user.email "hsuyiping@gmail.com"
   git config user.name "hsuyiping-rgb"
...
9. **建立私有 repo**：問使用者偏好的英文 repo 名，然後
   ```bash
   gh repo create hsuyiping-rgb/<repo-name> --private --source=. --push
   ```
...
✅ L2 GitHub：hsuyiping-rgb/<repo>（私有）
...
- 所有訊息與檔案內容使用**繁體中文**
- 本 skill 在 `C:\Users\vm\.gemini\config\skills\`，搭配 chezmoi 跨電腦同步；修改後記得 `chezmoi re-add C:\Users\vm\.gemini\config\skills\project-init/`
